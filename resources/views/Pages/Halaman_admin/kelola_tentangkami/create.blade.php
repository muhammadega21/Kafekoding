...
@section('content')
    ...
    <main class="h-full pb-16 overflow-y-auto">
        ...
        <section class="max-w-[990px] p-6 mx-auto bg-slate-100 rounded-md shadow-md dark:bg-gray-800 mt-10">
            <h1 class="text-xl font-bold text-slate-900 capitalize dark:text-slate-900">Tambah Data tentangkami</h1>
            @if (session('success'))
            ...
            <form action="{{ route('tentangkami.store') }}" method="post" enctype="multipart/form-data">
               <div class="grid grid-cols-1 sm:grid-cols-12">
                @csrf
                <div class="w-72">
                    <div class="relative h-[160px] w-full min-w-[900px] mt-5" >
                        <label class="block mt-4 text-sm">
                            ...
                            <textarea
                              ...
                              rows="5"
                              placeholder="Deskripsi" name="deskripsi_tentangkami"
                            ></textarea>
                          </label>
                    </div>
                </div>
                    ...
                    <div class="w-72">
                        <div class="relative h-10 w-full min-w-[350px] mt-5">
                            <label class="block mt-4 text-sm">
                                ...
                                <select name="status_tentangkami" id="status_tentangkami"
                                class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-[#203e59] focus:outline-none focus:shadow-outline-[#203e59] dark:focus:shadow-outline-gray" @error('status_tentangkami') is-invalid bg-danger @enderror>
                                <option value="">pilih jenis status</option>
                                <option value="aktif" @selected(old('status_tentangkami') === 'aktif')>Aktif</option>
                                <option value="tidak" @selected(old('status_tentangkami') === 'tidak')>Tidak</option>
                            </select>
    
                            @error('status_tentangkami')
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="file-upload-{{ $i }}"
                                            ...
                                            <input id="file-upload-{{ $i }}" name="gambar{{ $i }}_tentangkami" type="file"
                                                class="sr-only">
                                        </label>
                                    </div>
                                </div>
