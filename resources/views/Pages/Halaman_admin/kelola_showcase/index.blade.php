            ...
            <table id="example" class="display border border-slate-200 " style="width:100%">
                <thead>
                    <tr >
                        ...
                        <th class="border border-slate-200  text-center">Bidang</th>
                    ...
                    @foreach ($showcase as $baris)
                        <tr>
                            ...
                            <td class="border border-slate-200 whitespace-normal break-all text-center">
                                {{ $baris->deskripsi_showcase }}
                            </td>
                            <td class="border border-slate-200 whitespace-normal break-all text-center">
                                {{ $baris->bidang_showcase }}
                            </td>
                            <td class="border border-slate-200 whitespace-normal break-all text-center">
                                <a href="{{ $baris->link_showcase }}" style="color: blue; text-decoration: underline;">{{ $baris->link_showcase }}</a>
                            </td>
                                ...
                                <div class="flex items-center justify-center text-sm">
                                    ...
                                    <div class="relative hidden w-32 h-32 rounded-full md:block">
                                        <img class="object-cover w-full h-full rounded-lg"
                                            src="{{ asset($baris->gambar_showcase) }}" alt="" loading="lazy" />
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="border border-slate-200">
                                <div class="flex items-center justify-center text-sm">
                                    <a href="{{ route('showcase.edit', $baris->id_showcase) }}">
                                    ...
                                    <form action="{{ route('showcase.destroy', $baris->id_showcase) }}" method="post" style="display: inline-block">
                                        @csrf
                                        @method('DELETE')
